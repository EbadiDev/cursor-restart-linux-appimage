...
for cmd in python3 uuidgen dmidecode; do
...
# Create a fake dmidecode command
FAKE_COMMANDS_DIR="$REAL_HOME/fake-commands"
mkdir -p "$FAKE_COMMANDS_DIR" || {
    echo "Error: Unable to create fake commands directory."
    exit 1
}

cat > "$FAKE_COMMANDS_DIR/dmidecode" << 'EOF'
#!/bin/bash
if [[ "$*" == *"--string-product-uuid"* ]]; then
    UUID=$(uuidgen)
    echo "$UUID"
else
    exec /usr/sbin/dmidecode "$@"
fi
EOF

chmod +x "$FAKE_COMMANDS_DIR/dmidecode" || {
    echo "Error: Unable to set permissions for the fake dmidecode command."
    exit 1
}
...
chown -R $REAL_USER:$(id -gn $REAL_USER) "$FAKE_COMMANDS_DIR" || {
    echo "Warning: Unable to change ownership of the fake commands directory."
}
...
echo "Fake dmidecode command created at: $FAKE_COMMANDS_DIR/dmidecode"
